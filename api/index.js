...
var initialOrders = [
  ...
  },
  {
    ...
    created_at: new Date(Date.now() - 28 * 36e5).toISOString()
  }
];
var initialReviews = [
  {
...
app.delete("/api/products/:id", async (req, res) => {
  ...
  store.products[index].updated_at = (/* @__PURE__ */ new Date()).toISOString();
  await saveData(store);
...
app.get("/api/orders/track", async (req, res) => {
  ...
  const searchTarget = order_number || query;
  const cleanTarget = searchTarget.trim().toUpperCase();
  const found = store.orders.find(
    (o) => o.order_number.toUpperCase() === cleanTarget || o.order_number.replace(/[^A-Z0-9]/g, "") === cleanTarget.replace(/[^A-Z0-9]/g, "")
  );
  if (!found) {
